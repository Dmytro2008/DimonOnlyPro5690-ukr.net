...
##        return (100*(x1**2 - x2)**2) + (x1 - 1)**2
##    else:
##        return (100*(x1**2 - x2)**2) + (x1 - 1)**2 + R*(((x1)**2 + x2**2)**2)
...
def grad_right(f, x, h, R):
    dx = (f(x[0]+h, x[1], R) - f(x[0], x[1], R))/h
    dy = (f(x[0], x[1]+h, R) - f(x[0], x[1], R))/h
    return np.array([dx, dy])


def grad_left(f, x, h, R):
    dx = (f(x[0], x[1], R) - f(x[0]-h, x[1], R))/h
    dy = (f(x[0], x[1], R) - f(x[0], x[1]-h, R))/h
    return np.array([dx, dy])
...
def plot_path(way):
    ...
    x_coord, y_coord = np.meshgrid(x_vals, y_vals)
##    z_coord = (100*(x_coord**2 - y_coord**2)**2) + (x_coord - 1)**2
##    (100*(x_coord**2 - y_coord**2)**2) + (x_coord - 1)**2
    z_coord =(x_coord-1)**2 + (y_coord-1)**2
...
def Koshi1(x0, epsilon, h_epsilon, odnom_epsilon, case, R):
    x = [np.array(x0)]
    end = False
    k=0
    while not end:
        grad = grad_center(x[k], h_epsilon, R)
        S = -grad
        svenn = svenn_la2(S, x[k], 0.001, case, 0, R)
        if case == 1:
            odnom_pousk = gold(x[k], svenn, S, odnom_epsilon, R)
        else:
            odnom_pousk = dsk_powell(x[k], svenn, S, odnom_epsilon, R)
        la_opt = odnom_pousk
        x_new = x[k] + la_opt*(S)
        x.append(x_new)
        if(np.linalg.norm(x[k]-x[k+1])/np.linalg.norm(x[k])<epsilon) \
                and ((evaluate_f(x[k][0],x[k][1], R)-evaluate_f(x[k+1][0],x[k+1][1], R))<epsilon):
            break
        k +=1
    return x
...
while end != True:
    x_k = x_k_list[-1]
    x = Partan(x_k, 0.0001, 0.01 , 0.01, 1, R)
